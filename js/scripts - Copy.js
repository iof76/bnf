	...
	$(document).ready(function(){
		...
		*/
		...
		function projectItems(){
			...
			if (window.matchMedia("(max-width: 575px)").matches && !el.hasClass("items-movil")) {
		...
		function displayItems(totalItems,inicioItems){
			...
			$.ajax({
			        ...
			        async: false,
			        type: "POST",
			        data: 'action=0&JSONInicio='+inicioItems+'&JSONTotal='+totalItems,
			        dataType: "json",
			    ...
			    if(outPutData!="0")
			    {
			    	...
			    	if(totalItems==1){
                        ...
                        contentDiv += outPutData[0].nombre;
                        contentDiv += "</figcaption>";
                        contentDiv += "</div>";
                        contentDiv += "<div class='mask'></div>";
                        contentDiv += "</a>"
                        contentDiv += "<div class='item-img'><img src='img/proj/"+outPutData[0].imagen+"_thumb.jpg' alt='Imagen de "+outPutData[0].nombre+"' /></div>";
                        contentDiv += "</figure>";
						...
						for (i=0; i<outPutData.length;i++){
	                        ...
	                        contentDiv += "<div class='item-info'>";
	                        contentDiv += "<figcaption class='headline'>";
	                        contentDiv += outPutData[i].nombre;
	                        contentDiv += "</figcaption>";
	                        contentDiv += "</div>";
	                        contentDiv += "<div class='mask'></div>";
	                        contentDiv += "</a>"
	                        contentDiv += "<div class='item-img'><img src='img/proj/"+outPutData[i].imagen+"_thumb.jpg' alt='Imagen de "+outPutData[i].nombre+"' /></div>";
	                        contentDiv += "</figure>";
			    		...
			    		var contentDiv = "<div class='wrap wrap-items-p'>";
						contentDiv +=" <div class='item-arrow-v'><span class='arrow' onclick='downItems("+inicioItems+","+totalItems+",3);'>z</span></div>";
						contentDiv +=" <div>";
						for (i=0; i<outPutData.length;i++){
							contentDiv += "<figure class='item'>";
